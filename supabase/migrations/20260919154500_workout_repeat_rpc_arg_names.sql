-- Parametri senza underscore: la cache API li espone così.
-- DROP necessario: stesso nome + stessi tipi, ma nomi argomento diversi.

DROP FUNCTION IF EXISTS public.set_workout_repeat_target(uuid, integer);
DROP FUNCTION IF EXISTS public.apply_workout_repeat_completion(uuid);

CREATE FUNCTION public.set_workout_repeat_target(
  workout_id uuid,
...
BEGIN
  ...
  t := GREATEST(1, LEAST(60, COALESCE(repeat_target, 1)));
...
CREATE FUNCTION public.apply_workout_repeat_completion(workout_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF NOT finished THEN
    ...
    WHERE workout_exercise_id IN (
      SELECT we.id FROM public.workout_exercises we WHERE we.workout_id = workout_id
    );
  END IF;
...
GRANT EXECUTE ON FUNCTION public.set_workout_repeat_target(uuid, integer) TO service_role;
