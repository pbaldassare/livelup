-- =====================================================
-- Riscaldamento / Defaticamento come template riusabili
-- template_role: main | warmup | cooldown
-- Collegamento opzionale su scheda main + phase su esercizi assegnati
-- =====================================================
...
-- 2) Flag + FK verso template warmup/cooldown (solo per role=main)
ALTER TABLE public.workout_templates
...
-- 3) Phase su esercizi/blocchi assegnati (snapshot)
...
COMMENT ON COLUMN public.workout_templates.template_role IS
  'main = scheda; warmup = riscaldamento; cooldown = defaticamento';
COMMENT ON COLUMN public.workout_exercises.phase IS
  'Fase snapshot: warmup/main/cooldown. Warmup/cooldown esclusi dal riepilogo sessione.';
