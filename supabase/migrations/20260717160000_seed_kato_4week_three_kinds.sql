...
-- 3 schede (libera / propedeutica / progressiva), max 3 esercizi ciascuna
-- Rotazione A→B→C, Lun/Mer/Ven, 4 settimane (12 allenamenti)
-- Idempotente (ri-eseguibile).
-- =============================================================================

-- Assicura colonne tipologia (no-op se già presenti)
...
DO $$
DECLARE
  v_athlete_email TEXT := 'kato.aifp@gmail.com';
  v_pt_fallback UUID := '76c207f5-ba7d-48d7-a7f2-c95f4819aebd'; -- Marco Ferrari
  v_athlete UUID;
  v_pt UUID;
  v_program_name TEXT := 'Demo 4 settimane — 3 tipologie';
  v_seed_tag TEXT := '[seed:kato-4w-kinds]';
  v_tpl_libera UUID;
  v_tpl_prop UUID;
  v_tpl_prog UUID;
  v_program UUID;
...
BEGIN
  ...
  -- 2) PT: connessione attiva, altrimenti Marco Ferrari
  ...
  -- 3) Catalogo esercizi (9 id, con fallback)
  ...
  -- helper locale: upsert template + 3 esercizi
  -- A libera
  SELECT id INTO v_tpl_libera
  FROM public.workout_templates
  WHERE pt_user_id = v_pt AND title = 'A — Full Body Libera'
  LIMIT 1;
  ...
  FROM public.workout_templates
  WHERE pt_user_id = v_pt AND title = 'B — Full Body Propedeutica'
  LIMIT 1;
  ...
  FROM public.workout_templates
  WHERE pt_user_id = v_pt AND title = 'C — Full Body Progressiva'
  LIMIT 1;
  ...
  -- 4) Archivia programmi seed precedenti con stesso nome
  ...
  -- Cancella workout seed precedenti dello stesso atleta/pt
  ...
  -- 6) Date attive (start sempre incluso + lun/mer/ven)
  ...
  FOR v_i IN 1 .. cardinality(v_dates) LOOP
    v_d := v_dates[v_i];
    IF ((v_i - 1) % 3) = 0 THEN
      v_tpl := v_tpl_libera; v_kind := 'libera'; v_title := 'A — Full Body Libera';
    ELSIF ((v_i - 1) % 3) = 1 THEN
      v_tpl := v_tpl_prop; v_kind := 'propedeutica'; v_title := 'B — Full Body Propedeutica';
    ELSE
      v_tpl := v_tpl_prog; v_kind := 'progressiva'; v_title := 'C — Full Body Progressiva';
    END IF;
