...
--
-- On Cedi (transfer_athlete_to_pt):
--   - ownership STAYS with current owner (usually cedente)
--   - destination gets/keeps active connection
--   - owner keeps active connection (both can coach)
--   - transfer logged in pt_atleta_transfers
--
-- On Riprendi (recall_athlete_from_transfer):
--   - owner-only
--   - terminate destination connection; keep owner + ownership
--
-- Also: is_athlete_owner helper + harden atleta_pt_subscriptions
-- writes to owner-only; fix Ceduti list for multi-active connections.
-- =====================================================
...
-- Connected PTs (and athlete) can read who the owner is for UI gates
...
BEGIN
  ...
  -- Ensure ownership row exists (first cede / legacy); never move ownership away
  ...
  -- Revoke active collaborator assignments under current owner (cede replaces that path)
...
BEGIN
  ...
  -- End destination coaching; keep owner connection + ownership
  ...
  -- Ownership stays on titolare (ensure row)
  INSERT INTO public.pt_athlete_owners (atleta_user_id, owner_pt_user_id)
...
-- =====================================================
-- Ceduti / Riprendibili: join destinazione esplicita
-- (con entrambe le connessioni active non pickare il titolare)
-- =====================================================
...
-- Connected PT who is pt_user_id may still SELECT own rows
-- =====================================================
...
-- =====================================================
-- Data fix: already-ceded athletes under old "move ownership" model
-- Restore titolare = cedente; keep destination connection; re-activate owner.
-- =====================================================
...
-- Ensure both PTs have active connections for unrecalled cedes
DO $$
DECLARE
  r RECORD;
BEGIN
