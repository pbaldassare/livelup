-- =====================================================
-- GRUPPI: creazione, ricerca, chat, moderazione admin
-- =====================================================
...
-- -----------------------------------------------------
-- RPC: join via invite token (gruppi privati)
-- -----------------------------------------------------
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'location_name', v_group.location_name,
    'visibility', v_group.visibility,
    'members_count', v_group.members_count,
    'is_official', v_group.is_official
  );
END;
$$;
...
BEGIN
  ...
  IF v_group.status <> 'active' THEN
    RAISE EXCEPTION 'Gruppo non disponibile';
  END IF;
  ...
  INSERT INTO public.group_members (group_id, user_id, role, status)
  VALUES (_group_id, v_uid, 'member', 'active');
...
-- group_disciplines
CREATE POLICY "group_disciplines_select"
  ON public.group_disciplines FOR SELECT TO authenticated
...
CREATE POLICY "group_members_insert_self_public"
  ON public.group_members FOR INSERT TO authenticated
  WITH CHECK (
    user_id = auth.uid()
    AND EXISTS (
      SELECT 1 FROM public.groups g
      WHERE g.id = group_id AND g.status = 'active' AND g.visibility = 'public'
    )
  );
...
CREATE POLICY "group_messages_insert"
  ...
  WITH CHECK (
    ...
    AND (
      channel = 'general'
      OR (channel = 'announcements' AND public.is_group_admin(group_id, auth.uid()))
    )
  );
...
CREATE POLICY "Users can update own group images"
  ON storage.objects FOR UPDATE TO authenticated
  USING (bucket_id = 'group-images' AND (storage.foldername(name))[1] = auth.uid()::text);

CREATE POLICY "Users can delete own group images"
